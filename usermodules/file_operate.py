...
def get_files(floder_path):
    ...
    """
    pure_Path=pathlib.Path(floder_path)
    return list(map(lambda x:str(x.resolve()),filter(lambda x:not x.is_dir(),pure_Path.iterdir())))
...
def get_files_by_extension(floder_path='.', extension_name='pdf'):
    ...
    :return: 文件列表
    """
    return list(filter(lambda x:pathlib.Path(x).match('*.'+extension_name),get_files(floder_path)))
