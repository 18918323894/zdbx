...
class userGui(object):
    ...
    def read_config(self):
        ...
        if self._json_data["templatePath"].find('template.xlsx')>-1 and os.path.exists(self._json_data['templatePath']):
            self._app.setEntry('template_user_fileentry',self._json_data["templatePath"])                 
        if self._json_data["pdfPath"]!='' and os.path.exists(self._json_data['pdfPath']):
            self._app.setEntry('pdf_folder',self._json_data["pdfPath"])
    ...
    def config(self):
        ...
        with self._app.labelFrame('欢迎使用报销自动化程序', sticky='ew'):
            self._app.startLabelFrame('pdf_folder', row + 1, col, 2,label='pdf文件路径')
            ent=self._app.addDirectoryEntry('pdf_folder',0,0)
            ent.bind("<FocusOut>", self.set_pdf_folder_path)
            self._app.stopLabelFrame()

            self._app.startLabelFrame('template', row + 2, col,2, label='模板文件路径')
            ...
            self._app.addButtons(["生成", "关闭"], self.press, row+7, col,2 ) # Row 3, Column 0, Span 2


        self._app.stopSubWindow()
        if os.path.exists(self._config_path):
            self.read_config()
    def set_pdf_folder_path(self,event):
        self._json_data['pdfPath']=self._app.getEntry('pdf_folder')
    ...
    def open_file_dialog(self,title):
        ...
        # 检查文件是否正确
        check_path=''
        if title.find('template')>-1:
            check_path=title.split('_user_fileentry')[0]+'.xlsx'

        if check_path !='':
            dir=self._app.openBox(title)

            if dir.find(check_path)>-1:
                self._json_data['templatePath']=dir
                self._app.setEntry('template_user_fileentry',dir)
            else:
    ...
    def press(self, btn):
        ...
        if btn == "生成":
            ...
            if all(entries.values()):
                pdfPath=self._app.getEntry('pdf_folder')
                if not cvt_pdf(pdfPath):
                    self._app.warningBox('警告','转换失败')
                else:
                    self._app.warningBox('','转换成功')
            else:
